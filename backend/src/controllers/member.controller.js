...
exports.applyMembership = async (req, res) => {
  try {
    ...
    let alAzharCertificate = existingMember
      ? existingMember.alAzharCertificate
      : { public_id: "", url: "" };
    const certFile =
      req.files && req.files["alAzharCertificate"]
        ? req.files["alAzharCertificate"][0]
        : null;
    if (certFile) {
...
exports.getAllMembers = async (req, res) => {
  try {
    const members = await Member.find()
      .populate(
        "user",
        "fullName email mobile role profileImage dob address state district",
      )
...
exports.approveMember = async (req, res) => {
  try {
    ...
    const verificationLink = `${process.env.FRONTEND_URL || "https://azhargraduatesindia.org/"}/verify-member/${member.memberId}`;
  ...
  try {
    ...
    res.status(200).json({
      ...
      member: {
        ...
        profileImage:
          member.profileImage?.url || member.user?.profileImage?.url || "",
