...
export function Logo({ variant = "dark", showText = true, className = "" }) {
  ...
  return (
    <Link href="/" className={`flex items-center gap-2 sm:gap-3 shrink-0 ${className}`} aria-label={`${shortName} home`}>
      <Image
      ...
      {showText && (
        <span className="notranslate flex flex-col leading-tight min-w-0">
          <span className={`font-serif text-sm sm:text-base font-bold tracking-tight ${textColor} truncate uppercase`}>{topText}</span>
          <span className={`text-[9px] sm:text-[10px] font-bold uppercase tracking-wider sm:tracking-[0.14em] ${subColor} truncate`}>
            {bottomText}
          </span>
        </span>
      )}
